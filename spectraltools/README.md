...
**Ultraviolet range**. It is possible to enable the option that takes into consideration the ultraviolet range in the making of the spectral files (`Tmake_spectralfile.py` enable UV = True). **You will need to upload the cross-sections and wavenumbers (same file) for each molecule yourself**. It's required to have a resolution of 0.01cm-1 in order to match the resolution socrates uses. If you are using DACE files for longer wavelengths, make sure that your ultraviolet files end where DACE starts (if there is not enough data, negligible values can be used).
